...
import it.moneymanagement.UI.models.TransazioniEntrataTableModel;
...
public class CercaEntrataPanel extends JPanel {

	private static final long serialVersionUID = 1L;
	private Logger logger = LoggerFactory.getLogger(CercaEntrataPanel.class);
	...
	private TransazioniEntrataTableModel transazioniTableModel;
	...
	protected void jButton0ActionActionPerformed(ActionEvent event) {
		...
		for (IncomeEntity entrata : entrate) {
			getTransazioniTableModel().addRow(new Object[] { entrata.getId(), Costanti.ENTRATA_KEY, 
		...
		if(getTabella().getColumnModel().getColumnCount()==7) {
		...
		ButtonColumn buttonColumn = new ButtonColumn(getTabella(), getDeleteFromTable(), 4, new ImageIcon("resources/images/cross.png"));
	...
	private JTable getTabella() {
		if (tabella == null) {
			...
			if(tabella.getColumnModel().getColumnCount()==7) {
			...
			tabella.getColumnModel().getColumn(4).setMaxWidth(10);
			tabella.getColumnModel().getColumn(4).setPreferredWidth(10);
		}
		return tabella;
	}
	...
	public TransazioniEntrataTableModel getTransazioniTableModel() {
		return transazioniTableModel;
	}

	public void setTransazioniTableModel(TransazioniEntrataTableModel transazioniTableModel) {
