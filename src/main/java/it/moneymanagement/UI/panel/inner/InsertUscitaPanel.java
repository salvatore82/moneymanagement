...
public class InsertUscitaPanel extends JPanel {

	private static final long serialVersionUID = 1L;
	private Logger logger = LoggerFactory.getLogger(InsertUscitaPanel.class);
	...
	private JCheckBox obbligatoria;
	...
	public InsertUscitaPanel() {
	}

	public void initComponents() {
		setBackground(Color.WHITE);
		//FORM DI INSERIMENTO
		JPanel form = new JPanel(new GridLayout(6, 3));
		...
		JLabel obbligatoriaLabel = new JLabel(getMessageSource().getMessage("label.obbligatoria", null, "Obbligatoria" , Locale.getDefault()),JLabel.RIGHT); 
		setLabelStyle(obbligatoriaLabel);
		form.add(obbligatoriaLabel);
		form.add(getObbligatoria());
		form.add(new JLabel(" "));
		JLabel extraLabel = new JLabel(getMessageSource().getMessage("label.extra", null, "Extra" , Locale.getDefault()),JLabel.RIGHT); 
		setLabelStyle(extraLabel);
		form.add(extraLabel);
		form.add(getExtra());
		form.add(new JLabel(" "));
		...
		//verticalGroup.setBorder(BorderFactory.createLineBorder(new Color(195, 37, 37)));
		...
		south.setBackground(new Color(195, 37, 37));
	...
	protected void jButton0ActionActionPerformed(ActionEvent event) {
		...
		outcomeEntity.setMandatory(getObbligatoria().getSelectedObjects() != null ? Short.valueOf("1") : Short.valueOf("0"));
		
		// Controllo extra
		outcomeEntity.setExtra(getExtra().getSelectedObjects() != null ? Short.valueOf("1") : Short.valueOf("0"));
		...
		if (esitoOperazione) {
			try {
				...
				if(selected == 0) {
					getBusinessDelegate().insertTransaxion(outcomeEntity.getTransaxion());
					getBusinessDelegate().insertOutcome(outcomeEntity);
	...
	private void resetFormComponents() {
		...
		getObbligatoria().setSelected(false);
		getExtra().setSelected(false);
		resetFormMessages();
	}
	...
	public JCheckBox getObbligatoria() {
		if (obbligatoria == null) {
			obbligatoria = new JCheckBox();
		}
		return obbligatoria;
	}
